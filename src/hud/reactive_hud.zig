...
pub const ReactiveHudData = struct {
    ...
    fn onRender(state: *anyopaque) !void {
        ...
        const log = std.log.scoped(.reactive_hud);
        log.debug("Reactive HUD render triggered - path: {s}, open: {}", .{ 
            self.current_path.peek(), 
            self.is_open.peek() 
        });
    }
...
pub const ReactiveHud = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, base_renderer: *game_renderer.GameRenderer) !Self {
        const hud_data = try ReactiveHudData.init(allocator, base_renderer);
        
        const component = try createComponent(
            ReactiveHudData,
            allocator,
            hud_data,
            ReactiveHudData.vtable
        );
    ...
    pub fn render(self: *Self, cmd_buffer: *c.sdl.SDL_GPUCommandBuffer, render_pass: *c.sdl.SDL_GPURenderPass) !void {
        ...
        try hud_data.renderer.renderNavigationBar(
            cmd_buffer,
            render_pass,
            hud_data.current_path.peek(),
            hud_data.can_go_back.get(),
            hud_data.can_go_forward.get()
        );
        ...
        try hud_data.renderer.renderLinks(
            cmd_buffer, 
            render_pass, 
            hud_data.links.items, 
            hud_data.hovered_link.peek()
        );
