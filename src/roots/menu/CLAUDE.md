...
SvelteKit-style routing system for in-game menus. Part of the `src/roots/` directory structure that contains base directories for separate UI document structures. Pages are defined by `+page.zig` files with filesystem-based routing.
...
- `/` → `src/roots/menu/+page.zig`
- `/settings` → `src/roots/menu/settings/+page.zig`
- `/settings/video` → `src/roots/menu/settings/video/+page.zig`
...
```bash
mkdir -p src/roots/menu/inventory
```

### Step 2: Create +page.zig
```zig
// src/roots/menu/inventory/+page.zig
const std = @import("std");
const page = @import("../../browser/page.zig");
...
Edit `src/hud/router.zig` and add:
```zig
...
```zig
const inventory_page = @import("../roots/menu/inventory/+page.zig");
```
...
### Adding a New Page
1. Create directory: `mkdir -p src/roots/menu/newpage`
...
## Roots Directory Concept

The `src/roots/` directory supports multiple "apps" in one codebase without privileging any:

- **No Single Privileged App**: Each root directory (like `menu/`) is an equal participant
- **SvelteKit-Style Routing**: Each root follows filesystem-based routing conventions
- **Clean Separation**: Different UI concerns are organized in separate root directories
- **Extensible**: Future UI "apps" can be added as new root directories

**Examples of potential roots:**
- `src/roots/menu/` - In-game menu system (current)
- `src/roots/editor/` - Level editor interface (future)
- `src/roots/debug/` - Debug dashboard (future)
- `src/roots/settings/` - Standalone settings app (future)
