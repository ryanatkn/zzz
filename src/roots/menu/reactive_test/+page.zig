...
pub const ReactiveTestPage = struct {
        ...
        test_page.current_effect = try reactive.createEffect(allocator, struct {
            fn run() void {
                ...
                const msg = std.fmt.allocPrint(
                    PageData.page_ptr.arena.allocator(),
                    "Count: {}, Doubled: {}",
                    .{ count, doubled }
                ) catch return;
        ...
        test_page.gannaway_watcher = try gannaway.watch(allocator, .{
            ...
            .callback = struct {
                fn onChange(changed: *const gannaway.Watchable) void {
                    ...
                    const msg = std.fmt.allocPrint(
                        GannawayData.page_ptr.arena.allocator(),
                        "Count: {}, Doubled: {}",
                        .{ count, doubled }
                    ) catch return;
    ...
    fn render(self: *const page.Page, links: *std.ArrayList(page.Link), arena: std.mem.Allocator) !void {
        ...
        try links.append(page.createLink(
            "REACTIVE SYSTEM COMPARISON TEST",
            "",
            screen_width / 2.0 - 300,
            50,
            600,
            60
        ));
        ...
        try links.append(page.createLink(
            "Increment",
            "/reactive-test?current_increment",
            left_x,
            current_y + 50,
            150,
            40
        ...
        for (mutable_page.current_log.items[current_log_start..], 0..) |log_entry, i| {
            ...
            try links.append(page.createLink(
                ...
                "",
                left_x + 20,
                current_y + 210 + @as(f32, @floatFromInt(i)) * 30,
                450,
                25
            ));
        }
        ...
        try links.append(page.createLink(
            "Increment",
            "/reactive-test?gannaway_increment",
            right_x,
            gannaway_y + 50,
            150,
            40
        ...
        for (mutable_page.gannaway_log.items[gannaway_log_start..], 0..) |log_entry, i| {
            ...
            try links.append(page.createLink(
                ...
                "",
                right_x + 20,
                gannaway_y + 210 + @as(f32, @floatFromInt(i)) * 30,
                450,
                25
            ));
        }
