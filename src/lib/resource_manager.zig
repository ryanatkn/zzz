...
pub const GPUResourceHelper = struct {
    ...
    pub fn createGameWindow(title: [*:0]const u8, width: i32, height: i32) ResourceError!*c.sdl.SDL_Window {
        return c.sdl.SDL_CreateWindow(title, width, height, c.sdl.SDL_WINDOW_VULKAN) orelse {
            const log = std.log.scoped(.resource_manager);
...
pub const ResourceLifecycle = struct {
    ...
    pub fn initGameResources(allocator: std.mem.Allocator, window_title: [*:0]const u8, window_width: i32, window_height: i32) ResourceError!GameResources {
