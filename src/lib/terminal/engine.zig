...
const ProcessExecutor = @import("capabilities/commands/executor.zig").Executor;
const CommandRegistry = @import("capabilities/commands/registry.zig").Registry;
const CommandContext = @import("capabilities/commands/registry.zig").CommandContext;
const AnsiParser = ansi.AnsiParser;
const ProcessControl = process_control.ProcessControl;
const SignalHandler = process_control.SignalHandler;
const Key = core.Key;
...
pub fn writeToTerminal(terminal_ptr: *anyopaque, text: []const u8) !void {
...
pub fn executeCommandCallback(context: *anyopaque, command: []const u8) !void {
    const ui_log = loggers.getUILog();
...
pub fn streamOutputCallback(context: *anyopaque, data: []const u8) !void {
    const engine: *TerminalEngine = @ptrCast(@alignCast(context));
