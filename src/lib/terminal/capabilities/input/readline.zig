...
pub const ReadlineInput = struct {
    ...
    pub fn initialize(self: *Self, deps: []const kernel.TypeSafeCapability, event_bus: *kernel.EventBus) !void {
        ...
        for (deps) |dep| {
            if (dep.cast(@import("../state/line_buffer.zig").LineBuffer)) |lb| {
                self.line_buffer = lb;
            } else if (dep.cast(@import("../state/cursor.zig").Cursor)) |c| {
                self.cursor = c;
            } else if (dep.cast(@import("keyboard.zig").KeyboardInput)) |ki| {
