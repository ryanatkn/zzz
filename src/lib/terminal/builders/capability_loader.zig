...
pub const CapabilityLoader = struct {
    ...
    pub fn validateCompatibility(self: *CapabilityLoader, capabilities: []const CapabilityType) !void {
            ...
            for (metadata.conflicts) |conflict| {
                for (capabilities) |other_cap| {
                    if (other_cap == conflict) {
                        std.log.err("Capability conflict: {s} conflicts with {s}", .{ metadata.name, BuiltinRegistry.getMetadata(conflict).name });
        ...
        for (capabilities) |cap_type| {
            ...
            for (metadata.dependencies) |dependency| {
                ...
                if (!found) {
                    std.log.err("Missing dependency: {s} requires {s}", .{ metadata.name, BuiltinRegistry.getMetadata(dependency).name });
    ...
    pub fn getRecommendedCapabilities(self: *CapabilityLoader, use_case: UseCase, recommended: *std.ArrayList(CapabilityType)) !void {
        ...
        const caps = switch (use_case) {
            ...
            .interactive_shell => &[_]CapabilityType{ .readline_input, .ansi_writer, .line_buffer, .cursor, .history, .screen_buffer, .scrollback, .persistence, .parser, .registry, .executor, .builtin, .pipeline },
            .logging_terminal => &[_]CapabilityType{ .keyboard_input, .buffered_output, .screen_buffer, .scrollback, .persistence },
            .embedded_terminal => &[_]CapabilityType{ .keyboard_input, .basic_writer, .line_buffer, .cursor },
        };
