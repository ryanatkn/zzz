<script lang="ts">
	...
	// Add function to handle provider data safely
	const handle_provider_data = (data: any): string => {
		if (!data || !data.type) return 'Unknown data type';
...
{#if completion_response}
	<div class="mb_sm">
		...
		{:else}
			{handle_provider_data(completion_response.data)}
		{/if}
	</div>
{:else}
