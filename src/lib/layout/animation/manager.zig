...
pub const AnimationManager = struct {
    springs: std.ArrayList(springs.LayoutSpring),
    transitions: std.ArrayList(transitions.LayoutTransition),
    sequences: std.ArrayList(sequencer.AnimationSequence),
    timelines: std.ArrayList(sequencer.AnimationTimeline),
    allocator: std.mem.Allocator,

    pub fn init(allocator: std.mem.Allocator) AnimationManager {
        return AnimationManager{
            .springs = std.ArrayList(springs.LayoutSpring).init(allocator),
            .transitions = std.ArrayList(transitions.LayoutTransition).init(allocator),
            .sequences = std.ArrayList(sequencer.AnimationSequence).init(allocator),
            .timelines = std.ArrayList(sequencer.AnimationTimeline).init(allocator),
            .allocator = allocator,
        };
    }
    ...
    pub fn addSpring(self: *AnimationManager, spring: springs.LayoutSpring) !*springs.LayoutSpring {
        try self.springs.append(spring);
        return &self.springs.items[self.springs.items.len - 1];
    }

    /// Add a new transition animation
    pub fn addTransition(self: *AnimationManager, transition: transitions.LayoutTransition) !*transitions.LayoutTransition {
    ...
    pub fn addSequence(self: *AnimationManager, sequence: sequencer.AnimationSequence) !*sequencer.AnimationSequence {
        try self.sequences.append(sequence);
        return &self.sequences.items[self.sequences.items.len - 1];
    }

    /// Add a new timeline
    pub fn addTimeline(self: *AnimationManager, timeline: sequencer.AnimationTimeline) !*sequencer.AnimationTimeline {
