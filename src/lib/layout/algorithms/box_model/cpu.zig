/// CPU implementation of CSS box model layout algorithm
...
pub const BoxModelCPU = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, position: Vec2, element_size: Vec2) !BoxModelCPU {
        return BoxModelCPU{
            .allocator = allocator,
            .position = position,
            .size = element_size,
    ...
    pub fn deinit(self: *BoxModelCPU, allocator: std.mem.Allocator) void {
        _ = allocator;
        _ = self;
    ...
    pub fn getLayout(self: *BoxModelCPU) ComputedLayout {
    ...
    pub fn setSize(self: *BoxModelCPU, size: Vec2) void {
        self.size = size;
        self.markDirty();
    }

    /// Set padding uniformly
    pub fn setPadding(self: *BoxModelCPU, padding: f32) void {
        self.padding = Spacing.uniform(padding);
        self.markDirty();
    }

    /// Set padding per side
    pub fn setPaddingDetailed(self: *BoxModelCPU, top: f32, right: f32, bottom: f32, left: f32) void {
        self.padding = Spacing{ .top = top, .right = right, .bottom = bottom, .left = left };
        self.markDirty();
    }

    /// Set margin uniformly
    pub fn setMargin(self: *BoxModelCPU, margin: f32) void {
        self.margin = Spacing.uniform(margin);
        self.markDirty();
    }

    /// Set constraints
    pub fn setConstraints(self: *BoxModelCPU, constraints: Constraints) void {
        self.constraints = constraints;
        self.markDirty();
    }

    /// Mark as needing recalculation
    pub fn markDirty(self: *BoxModelCPU) void {
        self.dirty = true;
        self.cached_layout = null;
    }

    /// Check if layout needs recalculation
    pub fn isDirty(self: *BoxModelCPU) bool {
        return self.dirty;
    }

    /// Clear dirty flag
    pub fn clearDirty(self: *BoxModelCPU) void {
        self.dirty = false;
    }
};
...
pub fn calculateBoxModel(position: Vec2, content_size: Vec2, padding: Spacing, margin: Spacing, border: Spacing) BoxModelCPU.ComputedLayout {
...
pub const BoxModelAlgorithm = struct {
    ...
    pub fn calculate(
        ...
        for (elements, 0..) |*element, i| {
            ...
            var box_model = try BoxModelCPU.init(allocator, element.position, element.size);
...
test "box model creation and basic properties" {
    ...
    var box = try BoxModelCPU.init(allocator, Vec2{ .x = 10, .y = 20 }, Vec2{ .x = 100, .y = 50 });
...
test "box model with padding" {
    ...
    var box = try BoxModelCPU.init(allocator, Vec2.ZERO, Vec2{ .x = 100, .y = 50 });
    defer box.deinit(allocator);
