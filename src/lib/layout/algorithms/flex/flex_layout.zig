...
const Vec2 = shared.Vec2;
const Rectangle = shared.Rectangle;
const Config = shared.Config;
const FlexItem = shared.FlexItem;
...
pub const FlexLayout = struct {
    ...
    pub fn calculateLayout(
        ...
        if (is_reverse) {
            shared.reverseResults(results, config.direction);
        }
    ...
    fn createFlexLines(
        ...
        for (items) |item| {
            const item_main_size = shared.getItemMainSize(item, config.direction);
    ...
    fn calculateCrossAxisPosition(
        ...
        return switch (alignment) {
            .start => 0,
            .end => container_cross_size - item_cross_size,
...
test "flex layout basic row layout" {
    ...
    const config = Config{ .direction = .row };
