...
const SpacingUtils = spacing.SpacingUtils;
const SizingUtils = sizing.SizingUtils;
const PositioningUtils = positioning.PositioningUtils;
...
pub const Flexbox = struct {
    ...
    pub const AlignItems = enum {
        ...
        baseline, // Align baselines (simplified implementation)
    };
    ...
    /// Layout result for a flex item
    pub const FlexItemLayout = struct {
        position: Vec2,
        size: Vec2,
    ...
    pub fn calculateLayout(
        ...
        if (items.len == 0) return try allocator.alloc(FlexItemLayout, 0);
        ...
        var flex_constraints = try allocator.alloc(SizingUtils.FlexConstraint, items.len);
        ...
        for (items, 0..) |item, i| {
            ...
            flex_constraints[i] = SizingUtils.FlexConstraint{
        ...
        var result = try allocator.alloc(FlexItemLayout, items.len);
        ...
        for (items, 0..) |item, i| {
            ...
            result[display_index] = FlexItemLayout{
    ...
    pub fn createRowLayout(justify: JustifyContent, align_items: AlignItems, gap: f32) Flexbox {
        return Flexbox{
            .direction = .row,
    ...
    pub fn createColumnLayout(justify: JustifyContent, align_items: AlignItems, gap: f32) Flexbox {
        return Flexbox{
            .direction = .column,
            .justify_content = justify,
            .align_items = align_items,
...
test "basic row layout" {
    ...
    const flex = Flexbox.createRowLayout(.flex_start, .flex_start, 10);

    const items = [_]Flexbox.FlexItem{
        Flexbox.createFlexItem(Vec2{ .x = 50, .y = 30 }, 0, 1),
        Flexbox.createFlexItem(Vec2{ .x = 60, .y = 40 }, 0, 1),
        Flexbox.createFlexItem(Vec2{ .x = 40, .y = 20 }, 0, 1),
    };
...
test "flex grow distribution" {
    ...
    const flex = Flexbox.createRowLayout(.flex_start, .flex_start, 0);
...
test "center justification" {
    ...
    const flex = Flexbox.createRowLayout(.center, .flex_start, 10);

    const items = [_]Flexbox.FlexItem{
        Flexbox.createFlexItem(Vec2{ .x = 40, .y = 30 }, 0, 1),
        Flexbox.createFlexItem(Vec2{ .x = 40, .y = 30 }, 0, 1),
    };
...
test "column layout" {
    ...
    const flex = Flexbox.createColumnLayout(.flex_start, .flex_start, 5);

    const items = [_]Flexbox.FlexItem{
        Flexbox.createFlexItem(Vec2{ .x = 50, .y = 30 }, 0, 1),
        Flexbox.createFlexItem(Vec2{ .x = 60, .y = 25 }, 0, 1),
    };
