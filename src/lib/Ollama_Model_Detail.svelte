...
<script lang="ts">
	...
	import {GLYPH_REFRESH, GLYPH_CHEVRON_DOWN, GLYPH_CHEVRON_RIGHT} from '$lib/glyphs.js';
...
<div class="display_flex flex_column">
	...
	<button
		type="button"
		class="display_flex justify_content_space_between align_items_center p_sm plain text_align_start"
		onclick={() => {
			show_details = !show_details;
			if (show_details && model_detail.show_status === 'initial') {
				void load_model_details();
			}
		}}
	>
		<span class="font_weight_600">details</span>
		<Glyph glyph={show_details ? GLYPH_CHEVRON_DOWN : GLYPH_CHEVRON_RIGHT} />
	</button>

	{#if show_details}
		<div class="p_md border_top" transition:slide>
				...
				<div class="display_flex flex_column gap_md">
					...
					{#if model_detail.show_response.model_info && model_detail.show_response.model_info instanceof Map && model_detail.show_response.model_info.size > 0}
						<div>
							...
							<pre
								class="font_size_sm bg_2 p_sm border_radius_xs overflow_auto"
								style:max-height="300px">{format_model_info(
									model_detail.show_response.model_info,
								).join('\n')}</pre>
						</div>
					{/if}
