...
export const gen: Gen = ({origin_path}) => {
	...
	if (needs_request_response) action_event_imports.push('Frontend_Request_Response_Action_Event');
	if (needs_remote_notification)
		action_event_imports.push('Frontend_Remote_Notification_Action_Event');
	if (needs_local_call) action_event_imports.push('Frontend_Local_Call_Action_Event');

	const frontend_action_handlers = registry.specs
		.map((s) => generate_frontend_phase_handlers(s))
		.join(';\n\t');
...
const generate_frontend_phase_handlers = (spec: Action_Spec): string => {
	const {method, kind, initiator} = spec;
	const all_phases = ACTION_KIND_PHASES[kind];

	// Determine which phases frontend can handle based on initiator
	const frontend_phases: Array<Action_Phase> = [];

	if (kind === 'request_response') {
		if (initiator === 'frontend' || initiator === 'both') {
			frontend_phases.push('send_request', 'receive_response');
		}
		if (initiator === 'backend' || initiator === 'both') {
			frontend_phases.push('receive_request', 'send_response');
		}
	} else if (kind === 'remote_notification') {
		if (initiator === 'frontend' || initiator === 'both') {
			frontend_phases.push('send');
		}
		if (initiator === 'backend' || initiator === 'both') {
			frontend_phases.push('receive');
		}
	} else if (kind === 'local_call') {
		if (initiator === 'frontend' || initiator === 'both') {
			frontend_phases.push('execute');
		}
	}

	if (frontend_phases.length === 0) {
		return `${method}?: never`;
	}
	...
	if (kind === 'request_response') {
		event_type = `Frontend_Request_Response_Action_Event<'${method}', Action_Inputs['${method}'], Action_Outputs['${method}']>`;
	} else if (kind === 'remote_notification') {
		event_type = `Frontend_Remote_Notification_Action_Event<'${method}', Action_Inputs['${method}']>`;
	} else {
		event_type = `Frontend_Local_Call_Action_Event<'${method}', Action_Inputs['${method}'], Action_Outputs['${method}']>`;
	}

	const phase_handlers = frontend_phases
