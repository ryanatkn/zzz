...
export interface Actions {
	${action_schemas_registry
		.map((schema) => {
			const hasParams =
				get_innermost_type(schema.params)._def.typeName !== z.ZodFirstPartyTypeKind.ZodNull;
	...
	${action_schemas_registry
		.map((schema) => {
			const responseType = is_service_action(schema)
				...
				: 'void';
			return `${schema.name}?: Mutation<${to_action_params_name(schema.name)}, ${responseType}>;`;
		})
		.join('\n\t')}
}
