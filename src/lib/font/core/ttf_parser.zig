...
pub const TTFParser = struct {
    ...
    pub fn deinit(self: *TTFParser) void {
        ...
        // Free kerning pairs if they were allocated
        if (self.kern) |kern_table| {
            self.allocator.free(kern_table.pairs);
        }
    }
    ...
    fn parseTables(self: *TTFParser) !void {
        ...
        // Parse kern table if available (optional)
        if (self.tables.get("kern")) |kern_record| {
            self.kern = self.parseKernTable(kern_record) catch null; // Ignore errors, kerning is optional
        }
    }
    ...
    fn parseKernTable(self: *TTFParser, record: TableRecord) !KernTable {
        const offset = record.offset;
        if (offset + 4 > self.data.len) return error.InvalidKernTable;

        const version = std.mem.readInt(u16, self.data[offset..][0..2], .big);
        const n_tables = std.mem.readInt(u16, self.data[offset + 2 ..][0..2], .big);

        // For now, we only support version 0 and read the first subtable
        if (version != 0 or n_tables == 0) return error.UnsupportedKernTable;

        const subtable_offset = offset + 4;
        if (subtable_offset + 6 > self.data.len) return error.InvalidKernTable;

        const subtable_version = std.mem.readInt(u16, self.data[subtable_offset..][0..2], .big);
        const subtable_length = std.mem.readInt(u16, self.data[subtable_offset + 2 ..][0..2], .big);
        const coverage = std.mem.readInt(u16, self.data[subtable_offset + 4 ..][0..2], .big);

        // Check if this is a horizontal kerning table (coverage bit 0 = 0)
        if ((coverage & 0x01) != 0) return error.UnsupportedKernTable;

        _ = subtable_version;
        _ = subtable_length;

        const pairs_offset = subtable_offset + 6;
        if (pairs_offset + 8 > self.data.len) return error.InvalidKernTable;

        const n_pairs = std.mem.readInt(u16, self.data[pairs_offset..][0..2], .big);

        var pairs = try self.allocator.alloc(KernPair, n_pairs);

        const pairs_data_offset = pairs_offset + 8; // Skip n_pairs + searchRange + entrySelector + rangeShift

        for (0..n_pairs) |i| {
            const pair_offset = pairs_data_offset + (i * 6);
            if (pair_offset + 6 > self.data.len) {
                self.allocator.free(pairs);
                return error.InvalidKernTable;
            }

            pairs[i] = KernPair{
                .left = std.mem.readInt(u16, self.data[pair_offset..][0..2], .big),
                .right = std.mem.readInt(u16, self.data[pair_offset + 2 ..][0..2], .big),
                .value = std.mem.readInt(i16, self.data[pair_offset + 4 ..][0..2], .big),
            };
        }
    ...
    pub fn getKerning(self: *TTFParser, left_glyph: u16, right_glyph: u16) i16 {
        const kern_table = self.kern orelse return 0;

        // Binary search through the kerning pairs
        var low: usize = 0;
        var high = kern_table.pairs.len;

        while (low < high) {
            const mid = (low + high) / 2;
            const pair = kern_table.pairs[mid];

            const pair_key = (@as(u32, pair.left) << 16) | pair.right;
            const search_key = (@as(u32, left_glyph) << 16) | right_glyph;

            if (pair_key == search_key) {
                return pair.value;
            } else if (pair_key < search_key) {
                low = mid + 1;
            } else {
                high = mid;
            }
