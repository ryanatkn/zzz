...
fn isPointInsideGlyph(test_x: f32, test_y: f32, contours: []const glyph_extractor.Contour, allocator: std.mem.Allocator) !bool {
    ...
    for (contours) |contour| {
        ...
        const tessellated_points = try curve_utils.tessellateContour(allocator, contour, curve_utils.DEFAULT_CONFIG);
...
pub const RasterizerCore = struct {
    ...
    pub fn rasterizeGlyph(
        ...
        for (outline.contours) |contour| {
            const tessellated_points = curve_utils.tessellateContour(self.allocator, contour, curve_utils.DEFAULT_CONFIG) catch continue;
