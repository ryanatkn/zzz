...
pub const PoolUtils = struct {
    ...
    pub fn createGamePools(allocator: std.mem.Allocator) !PoolManager {
        ...
        try manager.registerPool(GameEntity, "entities", 
            constants.PERFORMANCE.POOL_DEFAULT_SIZE, 
            constants.PERFORMANCE.POOL_MAX_SIZE);
        
        try manager.registerPool(Particle, "particles", 
            constants.RENDERING.MAX_PARTICLES, 
            constants.RENDERING.MAX_PARTICLES);
