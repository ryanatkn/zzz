...
pub const ShaderSet = struct {
    ...
    pub fn init(device: *c.sdl.SDL_GPUDevice) ShaderCreationError!ShaderSet {
        ...
        shaders.circle_ps = createFragmentShader(device, circle_ps_spv, "ps_main", 0) catch |err| {
        ...
        shaders.rect_ps = createFragmentShader(device, rect_ps_spv, "ps_main", 0) catch |err| {
        ...
        shaders.particle_ps = createFragmentShader(device, particle_ps_spv, "ps_main", 0) catch |err| {
...
fn createFragmentShader(device: *c.sdl.SDL_GPUDevice, spirv_data: []const u8, entrypoint: [*:0]const u8, uniform_buffers: u32) ShaderCreationError!*c.sdl.SDL_GPUShader {
    const shader_info = c.sdl.SDL_GPUShaderCreateInfo{
        .code_size = spirv_data.len,
        .code = @ptrCast(spirv_data.ptr),
        .entrypoint = entrypoint,
