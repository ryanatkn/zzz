...
pub const TextRenderer = struct {
    ...
    pub fn init(
        ...
        // Create instance buffer for batching (following circles pattern)
        const buffer_create_info = c.sdl.SDL_GPUBufferCreateInfo{
            .usage = c.sdl.SDL_GPU_BUFFERUSAGE_VERTEX,
            .size = uniforms_mod.MAX_INSTANCES_PER_BATCH * @sizeOf(TextInstance),
        };

        const instance_buffer = c.sdl.SDL_CreateGPUBuffer(device, &buffer_create_info) orelse {
            loggers.getRenderLog().err("text_buffer_fail", "Failed to create text instance buffer", .{});
