...
pub const TerminalRenderer = struct {
    ...
    pub fn render(self: *const Self, renderer: anytype, bounds: Rectangle, content: TerminalContent, header_text: ?[]const u8) !void {
        ...
        if (config.show_header and header_text != null) {
            const header_position = Vec2{ .x = content_area.position.x, .y = content_area.position.y };
    ...
    fn renderTerminalContent(self: *const Self, renderer: anytype, text_area: Rectangle, content: TerminalContent) !void {
        ...
        if (@hasDecl(@TypeOf(renderer), "drawRect")) {
            // Top border
            try renderer.drawRect(input_bg_rect.position, Vec2{ .x = input_bg_rect.size.x, .y = border_width }, border_color);
            // Bottom border
            try renderer.drawRect(Vec2{ .x = input_bg_rect.position.x, .y = input_bg_rect.position.y + input_bg_rect.size.y - border_width }, Vec2{ .x = input_bg_rect.size.x, .y = border_width }, border_color);
            // Left border
            try renderer.drawRect(input_bg_rect.position, Vec2{ .x = border_width, .y = input_bg_rect.size.y }, border_color);
            // Right border
            try renderer.drawRect(Vec2{ .x = input_bg_rect.position.x + input_bg_rect.size.x - border_width, .y = input_bg_rect.position.y }, Vec2{ .x = border_width, .y = input_bg_rect.size.y }, border_color);
        }

        // Render input text with padding
        const input_position = Vec2{ .x = text_area.position.x + side_margin + input_padding, .y = input_y + input_padding };
        try self.text_renderer.renderInputLine(renderer, content.prompt, content.current_input, content.cursor, input_position, text_area.size.x - (side_margin * 2) - (input_padding * 2));
