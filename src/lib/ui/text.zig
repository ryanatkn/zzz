...
pub const Text = struct {
    ...
    fn renderWithTextRenderer(self: *const Text, renderer: anytype, content: []const u8, position: Vec2, color: Color, style: TextStyle) !void {
        // Use the existing text_renderer system
        if (@hasDecl(@TypeOf(renderer), "font_manager")) {
            if (renderer.font_manager) |fm| {
                const text_result = fm.renderTextToTexture(content, style.font_category, style.font_size, color, renderer.device) catch {
    ...
    fn renderGeometric(self: *const Text, renderer: anytype, content: []const u8, position: Vec2, color: Color) !void {
        _ = self;
...
pub fn createText(allocator: std.mem.Allocator, content: []const u8, position: Vec2, style: TextStyle) !*Component {
...
pub fn createLabel(allocator: std.mem.Allocator, content: []const u8, position: Vec2, font_size: f32) !*Component {
...
pub fn createTitle(allocator: std.mem.Allocator, content: []const u8, position: Vec2, font_size: f32) !*Component {
    const style = TextStyle{
        .font_size = font_size,
        .color = Color{ .r = 255, .g = 255, .b = 255, .a = 255 }, // White
        .text_align = .center,
