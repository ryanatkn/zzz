...
pub const Panel = struct {
    ...
    pub fn getContentArea(self: *const Panel) Rectangle {
        ...
        return Rectangle{
            .position = Vec2{ 
                .x = bounds.position.x + pad, 
                .y = bounds.position.y + pad 
            },
            .size = Vec2{ 
                .x = bounds.size.x - (pad * 2), 
                .y = bounds.size.y - (pad * 2) 
            },
        };
    }
...
pub const Button = struct {
    ...
    pub fn render(self: *Button, renderer: anytype) void {
        ...
        const bg_color = if (!self.enabled.get()) self.disabled_color
        else if (self.pressed.get()) self.pressed_color
        else if (self.hovered.get()) self.hover_color
