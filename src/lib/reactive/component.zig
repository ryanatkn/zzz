...
pub const ReactiveComponent = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, component_state: *anyopaque, vtable: *const ComponentVTable) !Self {
...
pub fn createComponent(comptime T: type, allocator: std.mem.Allocator, component_data: T, comptime vtable: ReactiveComponent.ComponentVTable) !*ReactiveComponent {
    ...
    reactive_component.* = try ReactiveComponent.init(allocator, @as(*anyopaque, @ptrCast(component_ptr)), &vtable);
...
test "reactive component lifecycle" {
    ...
    var comp = try createComponent(TestComponent, allocator, TestComponent{ .name = "test" }, TestComponent.vtable);
    defer comp.deinit();
