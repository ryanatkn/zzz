...
pub fn withTracking(obs: *const observer.Observer, comptime func: fn () void) void {
    if (reactive_context) |ctx| {
...
pub const TrackingScope = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, scope_fn: utils.CallbackTypes.EffectFn) !Self {
        ...
        scope.scope_observer = observer.createObserver(TrackingScope, &scope, TrackingScope.onDependencyChange, TrackingScope.cleanup);
