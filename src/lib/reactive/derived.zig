...
pub fn Derived(comptime T: type) type {
    return struct {
        ...
        pub fn get(self: *Self) T {
            ...
            const dependency = context.createDependency(signal.Signal(T), &self.signal, signal.Signal(T).addObserver, signal.Signal(T).removeObserver);
...
pub fn derived(allocator: std.mem.Allocator, comptime T: type, derive_fn: utils.CallbackTypes.DeriveFn(T)) !*Derived(T) {
    ...
    deriv.observer = observer.createObserver(Derived(T), deriv, Derived(T).onDependencyChange, Derived(T).cleanup);
