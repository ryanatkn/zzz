...
pub fn createDependency(comptime T: type, signal: *T, comptime add_observer_fn: fn (*T, *const Observer) anyerror!void, comptime remove_observer_fn: fn (*T, *const Observer) void) Dependency {
...
pub fn createObserver(comptime T: type, effect: *T, comptime notify_fn: fn (*T) void, comptime cleanup_fn: ?fn (*T) void) Observer {
    const Wrapper = struct {
