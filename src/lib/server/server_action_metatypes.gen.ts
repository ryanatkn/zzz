...
export const gen: Gen = ({origin_path}) => {
	...
	const generate_server_phase_handlers = (spec: (typeof registry.specs)[0]): string => {
		...
		const phase_handlers = phases
			.map((phase: Action_Phase) => {
				...
				switch (phase) {
					case 'send_request':
						input_type = `typeof ${to_action_spec_input_identifier(method)}`;
						output_type = 'void';
					...
					case 'receive_request':
						...
						output_type =
							kind === 'request_response'
								? `typeof ${to_action_spec_output_identifier(method)}`
								: 'void';
						message_type = to_action_message_identifier(to_action_request_message_type(method));
						break;
					case 'send_response':
