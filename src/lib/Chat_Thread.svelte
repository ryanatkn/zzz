<script lang="ts">
	...
	import Turn_List from '$lib/Turn_List.svelte';
	...
	// TODO no longer uses `Chat`, maybe rename to `Thread_View` or similar?
...
<Model_Contextmenu model={thread.model}>
	<Thread_Contextmenu {thread}>
		<div {...attrs} class="chat_thread {attrs?.class}" class:empty class:dormant={!thread.enabled}>
			...
			{#if turn_count}
				<Turn_List {thread} attrs={turns_attrs} />
			{/if}
