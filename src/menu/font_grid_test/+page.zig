...
pub const FontGridTestPage = struct {
    ...
    fn render(self: *const page.Page, links: *std.ArrayList(page.Link)) !void {
        ...
        try links.append(page.createLink("Simple Bitmap", "#", // No click action - this is just a label
            start_x + 10.0, label_y, renderer_width - 20.0, 30.0));

        // Column 2: Oversampling 2x
        start_x += renderer_width;
        try links.append(page.createLink("Oversampling 2x", "#", start_x + 10.0, label_y, renderer_width - 20.0, 30.0));

        // Column 3: Oversampling 4x
        start_x += renderer_width;
        try links.append(page.createLink("Oversampling 4x", "#", start_x + 10.0, label_y, renderer_width - 20.0, 30.0));

        // Column 4: ASCII Debug
        start_x += renderer_width;
        try links.append(page.createLink("ASCII Debug", "#", start_x + 10.0, label_y, renderer_width - 20.0, 30.0));
        ...
        for (DEMO_TEXTS) |demo_text| {
            ...
            try links.append(page.createLink(demo_text, "#", start_x + 5.0, current_y, renderer_width - 10.0, text_height));
        ...
        try links.append(page.createLink("Back to Menu", "/", margin, nav_y, nav_width, 30.0));

        try links.append(page.createLink("Advanced Tests", "/font_test_comparison", margin + nav_width + nav_spacing, nav_y, nav_width, 30.0));
    }
