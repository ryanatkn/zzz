...
const SavePage = struct {
    ...
    fn render(self: *const page.Page, links: *std.ArrayList(page.Link), arena: std.mem.Allocator) !void {
        ...
        if (save_page.saved) {
            try links.append(page.createLink("✓ Settings saved successfully!", "", center_x - link_width / 2.0, y_pos, link_width, link_height));
            y_pos += link_height + link_spacing;

            try links.append(page.createLink("Saved to: font_settings.json", "", center_x - link_width / 2.0, y_pos, link_width, link_height));
        } else if (save_page.error_msg) |msg| {
            try links.append(page.createLink("✗ Failed to save settings", "", center_x - link_width / 2.0, y_pos, link_width, link_height));
            y_pos += link_height + link_spacing;

            try links.append(page.createLink(msg, "", center_x - link_width / 2.0, y_pos, link_width, link_height));
        }

        y_pos += link_height + link_spacing * 2;

        try links.append(page.createLink("Back to Font Settings", "/settings/fonts", center_x - link_width / 2.0, y_pos, link_width, link_height));
